...
# 在 Python 中，所有的整数都是 int 类型
a = 10
a = 20
# Python 中的整数的大小没有限制，可以是一个无限大的整数
a = 9999999999 ** 100 # 计算 9999999999 的 100 次幂
# 如果数字的长度过大，可以使用下划线作为分隔符
a = 123_456_789

# 10 进制的数字不能以 0 开头，如：a = 0123
...
# 浮点数（小数），在 Python 中所有的小数都是 float（浮点）类型
a = 1.23
a = 4.56

# 对浮点数进行运算时，可能会得到一个不精确的结果
a = 0.1 + 0.2 # 0.3000000000000004
...
# \n 表示换行符
# \\ 表示 \
# \uxxxx 表示 Unicode 编码，可以用来输入一些特殊字符
s = "子曰：\"学而时习之，不亦说乎\""
s = '\u0070'
...
s = 'abc' + 'cba'
# 字符串不能和其他的数据类型进行家法运算，如果做了会出现异常：TypeError: cannot concatenate 'str' and 'int' objects
print('s = ' + s) # 这种写法在 Python 中不常见
print('s = ', s) 
...
c = 'Hello %3.5s'%'abc' # %3.5s 字符串的长度限制在 3-5 之间
d = 'Hello %.2f'%123.456
e = 'Hello %d'%123.95
print(a)
print(b)
...
```python
# 布尔值主要用来做逻辑判断，布尔值一共有两个 True 和 False，True 表示真，Flase 表示假
a = True
b = False
print('a = ', a)

# 布尔值实际上也属于整形，True 就相当于 1，False 就相当于 0
print(1 + False)
```
